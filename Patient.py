...
class Patient:
    ...
    # Insert appointment with parameters caregiver and date d
    def make_appointment(self, caregiver, date, vaccine):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()

        add_appointment = "Insert Into Appointments Values (%d , %s , %s , %s)"
        try:
            cursor.execute(add_appointment, (date, self.username, caregiver, vaccine))
            conn.commit()
        except pymssql.Error:
            print("Error occured when making vaccine appointment")
            raise
        finally:
            cm.close_connection